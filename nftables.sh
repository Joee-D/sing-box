...
INTERFACE='lo'
TPROXY_PORT=7896 ## 和 sing-box 中定义的一致
...
setFirewallRules()
{
    ...
    nft add rule inet fw4 prerouting_tproxy meta l4proto {tcp,udp} tproxy to :$TPROXY_PORT meta mark set $PROXY_FWMARK ## 其他流量透明代理
	...
	#nft add rule inet fw4 nat_output meta nfproto {ipv4} counter jump output_tproxy
